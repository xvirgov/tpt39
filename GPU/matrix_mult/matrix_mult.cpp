...
float rand_float() {
  return floor(float(rand()) / float(RAND_MAX) * 20.0f);
}
...
printf ("GPU took %ld miliseconds to run. (ALL::[], COPY::[%ld], WRITE::[])\n", ms_kern, ms_cpy);
// Verify results.
bool pass = true;
